 ...
 * Usage: sentinel-audit scan --target <url|dir|host>
 *        sentinel-audit ls   (list available scanners)
 *        sentinel-audit check (installation check)
 */
...
async function runScanCommand(opts) {
  ...
  try {
    const result = await runScan({ target, targetDir, targetUrl, scopes: scopes || null, config });
    ...
    if (format === 'json') {
      await new JsonReporter().write(result, outputPath);
    } else {
      await new MarkdownReporter().write(result, outputPath);
    }
